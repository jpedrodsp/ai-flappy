...
class AiPlayer(Entity):
    def __init__(self, config: GameConfig) -> None:
        ...
        x = int(config.window.width * 0.2) - 0.2
        ...
        safe_distance_variation = int(max(1, 200 / config.generations))
        self.safedistance = max(
            1,
            config.safedistance
            + random.randint(-safe_distance_variation, safe_distance_variation),
        )
        ready_distance_variation = int(max(1, 200 / config.generations))
        self.readydistance = max(
            1,
            config.readydistance
            + random.randint(
                -ready_distance_variation, ready_distance_variation
            ),
        )
        self.pipes_passed = 0
